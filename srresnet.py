...
class Srresnet:
    ...
    def ResidualBlock(self, x, kernel_size, filter_size):
        ...
        x = tf.nn.atrous_conv2d(x,filters = weights['w1'], rate=2, padding='SAME')
        # x = tf.layers.batch_normalization(x, training=self.training)
        # x = tf.nn.relu(x)
        x = tf.contrib.keras.layers.PReLU(shared_axes=[1, 2])(x)
        #x = tf.nn.conv2d(x, weights['w2'], strides=[1,1,1,1], padding='SAME')
        x = tf.nn.atrous_conv2d(x,filters = weights['w2'], rate=2, padding='SAME')
        # x = tf.nn.relu(x)
        x = tf.contrib.keras.layers.PReLU(shared_axes=[1, 2])(x)
        # x = tf.layers.batch_normalization(x, training=self.training)
    ...
    def forward_branch_bine(self, input_LL, input_edge):
        ...
        for i in range(1, D+1):
            ...
            for j in range(1, C+1):
                ...
                tmp = tf.nn.atrous_conv2d(x_edge,filters = self._weightsR['w_R_%d_%d' %(i, j)], rate=2, padding='SAME')
                tmp = tf.nn.relu(tmp)
                x_edge = tf.concat([x_edge, tmp], axis=3)

            x_edge = tf.nn.atrous_conv2d(x_edge, filters=self._weightsR['w_R_%d_%d' % (i, C+1)], rate=2, padding='SAME')
