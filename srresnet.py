...
class Srresnet:
    ...
    def __init__(self, training, content_loss='mse', learning_rate=1e-4, num_blocks=16):
        ...
        if content_loss not in ['mse', 'L1','edge_loss_mse','edge_loss_L1','ssim_loss']:
            print('Invalid content loss function. Must be \'mse\', or \'L1_loss\'.')
            exit()
        self.content_loss = content_loss
    """參考Photo-Realistic Single Image Super-Resolution Using a Generative Adversarial Network這篇"""
    ...
    """參考Residual Dense Network for Image Super-Resolution這篇"""
    def RDBParams(self):
        weightsR = {}
        biasesR = {}
        D = self.num_blocks
    ...
    def forward_branch_bine(self, input_LL, input_edge):
        ...
        for i in range(1, D+1):
            ...
            rdb_in = tf.add(rdb_in, x_LL)#區塊連接
    ...
    def _content_loss(self, hr_swt, sr_pred_level_2, sr_pred_level_1):
        ...
        pred_level_2_LL = tf.stack([sr_pred_level_2[:,:,:,0],sr_pred_level_2[:,:,:,4],sr_pred_level_2[:,:,:,8]], axis=-1)
        ...
        if self.content_loss == 'edge_loss_L1':
            ...
            # print('level_2_loss +labmda*level_1_loss:',level_2_loss +labmda*level_1_loss)

            return level_2_loss +labmda*level_1_loss

        if self.content_loss == 'ssim_loss':#這個還沒改完，參考
        
            r = 0.4
            loss = 0
            l, m, h = 1, 1, 1
            l, m, h = l * r * r, l * r * (1 - r), l * (1 - r) * (1 - r)
            pred_level_1_LH = tf.stack([pred_level_1_edge[:,:,:,0],pred_level_1_edge[:,:,:,3],pred_level_1_edge[:,:,:,6]],axis=-1)
            pred_level_1_HL = tf.stack([pred_level_1_edge[:,:,:,1],pred_level_1_edge[:,:,:,4],pred_level_1_edge[:,:,:,7]],axis=-1)
            pred_level_1_HH = tf.stack([pred_level_1_edge[:,:,:,2],pred_level_1_edge[:,:,:,5],pred_level_1_edge[:,:,:,8]],axis=-1)
            
            pred_level_2_LH = tf.stack([pred_level_2_edge[:,:,:,0],pred_level_2_edge[:,:,:,3],pred_level_2_edge[:,:,:,6]],axis=-1)
            pred_level_2_HL = tf.stack([pred_level_2_edge[:,:,:,1],pred_level_2_edge[:,:,:,4],pred_level_2_edge[:,:,:,7]],axis=-1)
            pred_level_2_HH = tf.stack([pred_level_2_edge[:,:,:,2],pred_level_2_edge[:,:,:,5],pred_level_2_edge[:,:,:,8]],axis=-1)

            hr_level_1_LH = tf.stack([hr_level_1_edge[:,:,:,0],hr_level_1_edge[:,:,:,3],hr_level_1_edge[:,:,:,6]],axis=-1)
            hr_level_1_HL = tf.stack([hr_level_1_edge[:,:,:,1],hr_level_1_edge[:,:,:,4],hr_level_1_edge[:,:,:,7]],axis=-1)
            hr_level_1_HH = tf.stack([hr_level_1_edge[:,:,:,2],hr_level_1_edge[:,:,:,5],hr_level_1_edge[:,:,:,8]],axis=-1)
            
            hr_level_2_LH = tf.stack([hr_level_2_edge[:,:,:,0],hr_level_2_edge[:,:,:,3],hr_level_2_edge[:,:,:,6]],axis=-1)
            hr_level_2_HL = tf.stack([hr_level_2_edge[:,:,:,1],hr_level_2_edge[:,:,:,4],hr_level_2_edge[:,:,:,7]],axis=-1)
            hr_level_2_HH = tf.stack([hr_level_2_edge[:,:,:,2],hr_level_2_edge[:,:,:,5],hr_level_2_edge[:,:,:,8]],axis=-1)

            print('pred_level_1_LH:',pred_level_1_LH.shape)
            print(' hr_level_1_LH:', hr_level_1_LH.shape)

            # count=0
            for i in range(16):

                # level_1_loss = - tf.image.ssim(tf.expand_dims(pred_level_1_LH[:,:,:],axis=0), tf.expand_dims(hr_level_1_LH[:,:,:],axis=0),max_val=1)* m\
                # - tf.image.ssim(tf.expand_dims(pred_level_1_HL[:,:,:],axis=0),tf.expand_dims(hr_level_1_HL[:,:,:],axis=0) ,max_val=1) * m \
                # - tf.image.ssim(tf.expand_dims(pred_level_1_HH[:,:,:],axis=0),tf.expand_dims(hr_level_1_HH[:,:,:],axis=0),max_val=1) * h

                # level_2_loss = - tf.image.ssim(tf.expand_dims(pred_level_2_LH[:,:,:],axis=0),tf.expand_dims(hr_level_2_LH[:,:,:],axis=0),max_val=1)*l*l\
                # - tf.image.ssim(tf.expand_dims(pred_level_2_HL[:,:,:],axis=0),tf.expand_dims(hr_level_2_HL[:,:,:],axis=0),max_val=1) * m * l\
                # - tf.image.ssim(tf.expand_dims(pred_level_2_HH[:,:,:],axis=0),tf.expand_dims(hr_level_2_HH[:,:,:],axis=0),max_val=1) * h * l

                # loss += level_1_loss + level_2_loss - tf.image.ssim(tf.expand_dims(pred_level_2_LL[:,:,:],axis=0),tf.expand_dims(hr_level_2_LL[i,:,:,:],axis=0),max_val=1) * l * l 
                # # count+=1
                # print('level_1_loss:',level_1_loss)
                # print('level_2_loss:',level_2_loss)

                level_1_loss = - tf.image.ssim(pred_level_1_LH[i,:,:,:],hr_level_1_LH[i,:,:,:],max_val=1)* m\
                - tf.image.ssim(pred_level_1_HL[i,:,:,:],hr_level_1_HL[i,:,:,:],max_val=1) * m\
                - tf.image.ssim(pred_level_1_HH[i,:,:,:],hr_level_1_HH[i,:,:,:],max_val=1) * h

                level_2_loss = - tf.image.ssim(pred_level_2_LH[i,:,:,:],hr_level_2_LH[i,:,:,:],max_val=1)*m\
                - tf.image.ssim(pred_level_2_HL[i,:,:,:],hr_level_2_HL[i,:,:,:],max_val=1) * m\
                - tf.image.ssim(pred_level_2_HH[i,:,:,:],hr_level_2_HH[i,:,:,:],max_val=1) * h

                loss += level_1_loss + level_2_loss - tf.image.ssim(pred_level_2_LL[i,:,:,:],hr_level_2_LL[i,:,:,:],max_val=1) * l 
                # count+=1
                print('level_1_loss:',level_1_loss)
                print('level_2_loss:',level_2_loss)
                print('pred_level_1_LH',pred_level_1_LH[i,:,:,:].shape)

            return loss    

    def loss_function(self, hr_swt, y_pred_level_2, y_pred_level_1):

        # Content loss only
        return self._content_loss(hr_swt, y_pred_level_2, y_pred_level_1)
