...
def build_log_dir(args, arguments):
    ...
    with open(log_path + '/loss.csv', 'a') as f:
        f.write(
            'iteration, val_error, eval_error, Rain12_psnr, Rain12_ssim, Rain100L_psnr, Rain100L_ssim, Rain100H_psnr, Rain100H_ssim\n')
        # f.write(
            # 'iteration, Rain12_psnr, Rain12_ssim, Rain100L_psnr, Rain100L_ssim, Rain100H_psnr, Rain100H_ssim\n')
...
def evaluate_model(loss_function, get_batch_rain, get_batch_label, sess, num_images, batch_size):
    """Tests the model over all num_images using input tensor get_batch"""
    loss = 0
    total = 0
    for i in range(int(math.ceil(num_images / batch_size))):
        
        batch_label = batch_bgr2rgb(get_batch_label)
        batch_rain = batch_bgr2rgb(get_batch_rain)
       #batch_rain = downsample_batch(batch_label, factor=4)

        batch_dwt_label = batch_Swt(batch_label)
        batch_dwt_rain = batch_Swt(batch_rain)
        """norain的RGB低頻和rain的RGB低頻"""
        ...
        loss += sess.run(loss_function,
                         feed_dict={'srresnet_training:0': False,\
                                    'LR_DWT_A:0': batch_dwt_rain_A,\
                                    'LR_DWT_edge:0': batch_rain_BCD,\
                                    'HR_image:0': batch_dwt_label_A,\
                                    'HR_DWT_edge:0': batch_label_BCD,\
                                    'HR:0': batch_label,\
                                    })
        total += 1
    loss = loss / total
